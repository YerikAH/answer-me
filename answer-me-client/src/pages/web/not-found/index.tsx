import logo from "@/assets/logo.png";
import { useTitle } from "@/hooks";
import { ROUTES } from "@/router/routes";
import { useEffect } from "react";
import { Link } from "react-router-dom";

export default function NotFound() {
  const { changeTitle } = useTitle();
  useEffect(() => {
    changeTitle("Error 404 -  Arxatec");
  }, []);
  return (
    <div className="grid h-screen grid-cols-1 grid-rows-[1fr,auto,1fr] bg-white lg:grid-cols-[max(50%,36rem),1fr]">
      <header className="mx-auto w-full max-w-7xl px-6 pt-6 sm:pt-10 lg:col-span-2 lg:col-start-1 lg:row-start-1 lg:px-8">
        <Link to={ROUTES.DASHBOARD} className="flex items-center gap-2">
          <div className="border rounded-full flex overflow-hidden w-fit">
            <span className="sr-only">Your Company</span>
            <img alt="logo" src={logo} className="h-8 w-auto " />
          </div>
          <p className="text-sm text-zinc-900 font-semibold">Arxatec</p>
        </Link>
      </header>
      <main className="mx-auto w-full max-w-7xl px-6 py-24 sm:py-32 lg:col-span-2 lg:col-start-1 lg:row-start-2 lg:px-8">
        <div className="max-w-lg">
          <p className="text-7xl font-extrabold  text-zinc-200">404</p>
          <h1 className="mt-4 text-3xl font-bold tracking-tight text-gray-900 sm:text-5xl">
            Oops, esto no debería pasar...
          </h1>
          <p className="mt-6 text-base leading-7 text-zinc-600">
            Parece que la página que estás buscando no existe o ha sido movida.
            Por favor, verifica la URL o vuelve al inicio para seguir navegando.
          </p>
          <div className="mt-10">
            <Link
              to={ROUTES.DASHBOARD}
              className="text-sm font-semibold leading-7 text-white bg-zinc-900 px-4 py-2.5 rounded-lg hover:bg-zinc-700 transition-all"
            >
              <span aria-hidden="true">&larr;</span> Back to home
            </Link>
          </div>
        </div>
      </main>
      <footer className="self-end lg:col-span-2 lg:col-start-1 lg:row-start-3">
        <div className="border-t border-gray-100 bg-gray-50 py-10">
          <nav className="mx-auto flex w-full max-w-7xl items-center gap-x-4 px-6 text-sm leading-7 text-gray-600 lg:px-8">
            <a href="#">Contact support</a>
            <svg
              viewBox="0 0 2 2"
              aria-hidden="true"
              className="h-0.5 w-0.5 fill-gray-300"
            >
              <circle r={1} cx={1} cy={1} />
            </svg>
            <a href="#">Status</a>
          </nav>
        </div>
      </footer>
      <div className="hidden lg:relative lg:col-start-2 lg:row-start-1 lg:row-end-4 lg:block">
        <img
          alt=""
          src="https://images.pexels.com/photos/7781900/pexels-photo-7781900.jpeg"
          className="absolute inset-0 h-full w-full object-cover"
        />
      </div>
    </div>
  );
}
